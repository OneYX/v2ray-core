...
const (
	gfwipFile = "https://raw.githubusercontent.com/oneyx/apnic/master/gfwip.txt"
)

func main() {
	resp, err := http.Get(gfwipFile)
	common.Must(err)
	...
	file, err := os.OpenFile("gfwip.generated.go", os.O_WRONLY|os.O_TRUNC|os.O_CREATE, 0644)
	if err != nil {
		log.Fatalf("Failed to generate gfwip_data.go: %v", err)
	}
	defer file.Close()

	fmt.Fprintln(file, "package gfwip")

	fmt.Fprintln(file, "var GfwIPs = "+formatArray(ipbytes))
}
